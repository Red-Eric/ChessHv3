...
function createSimpleAccuracyDisplay(
  ...
  if (!document.getElementById("acc-display-styles")) {
    ...
    style.textContent = `
      ...
      .acc-card-inactive .acc-label  { opacity: 0.45; }
      .acc-card-inactive .acc-threat-dot { opacity: 0.3; }
      .acc-card-inactive .acc-stats-row { opacity: 0.3; }
      ...
      .acc-card-white .acc-stats-row { border-top-color: #ddddd8; }
      .acc-card-black .acc-stats-row { border-top-color: rgba(255,255,255,0.05); }
      ...
      .acc-card-white .acc-stat-count { color: #444442; }
      .acc-card-black .acc-stat-count { color: #aaaaaa; }
  ...
  const STAT_ICONS = [
    { key: "brilliant", svg: LogoBrillantSVG },
    { key: "greatFind", svg: LogogreatMoveSVG },
    { key: "bestMove", svg: LogobestMoveSVG },
    { key: "excellent", svg: LogoexcellentMoveSVG },
    { key: "good", svg: LogogoodMoveSVG },
    { key: "book", svg: LogobookSVG },
    { key: "forced", svg: LogoforcedSVG },
    { key: "inaccuracy", svg: LogoinaccuracyMoveSVG },
    { key: "mistake", svg: LogomistakeSVG },
    { key: "miss", svg: LogomissSVG },
    { key: "blunder", svg: LogoblunderSVG },
  ];

  function buildStatsHTML(stat) {
    if (!stat) return "";
    const items = STAT_ICONS.filter(
      ({ key }) => stat[key] != null && stat[key] > 0,
    )
      .map(
        ({ key, svg }) =>
          `<span class="acc-stat-item">${svg}<span class="acc-stat-count">${stat[key]}</span></span>`,
      )
      .join("");
    if (!items) return "";
    return `<div class="acc-stats-row">${items}</div>`;
  }
  ...
  function rowHTML(color, isYou) {
    ...
    const stat = color === "white" ? statW : statB;
    const statsHTML = buildStatsHTML(stat);
  ...
  function update(
    ...
    const statsChanged =
      (newStatW !== undefined && newStatW !== statW) ||
      (newStatB !== undefined && newStatB !== statB);

    if (newStatW !== undefined) statW = newStatW;
    if (newStatB !== undefined) statB = newStatB;

    // Rebuild DOM if side or stats changed
    if ((newSide !== undefined && newSide !== side) || statsChanged) {
      render();
    }
